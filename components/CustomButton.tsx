...
const CustomButton = ({
  ...
  return (
    <button
      type={`${buttonType}`}
      className={`${customWidth !== "no" && `w-${customWidth}`} uppercase bg-white px-${paddingX} py-${paddingY} text-${textSize} border border-black border-gray-300 font-bold text-amber-700 shadow-sm hover:bg-black hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-amber-500`}
    >
      {text}
    </button>
  );
