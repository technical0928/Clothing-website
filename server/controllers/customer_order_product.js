...
const createOrderProduct = asyncHandler(async (request, response) => {
  ...
  // Deduct stock when an item is added to an order
  if (existingProduct.inStock < parsedQuantity) {
    throw new AppError(
      `Only ${existingProduct.inStock} item(s) left in stock for "${existingProduct.title}"`,
      400
    );
  }
...
const deleteProductOrder = asyncHandler(async (request, response) => {
  ...
  // Restore stock before removing the order items
