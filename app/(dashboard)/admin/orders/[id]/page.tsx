        ...
        <div>
          {orderProducts?.map((product) => (
            <div className="flex items-center gap-x-4" key={product?.id}>
              <Image
                src={product?.product?.mainImage ? `/${product?.product?.mainImage}` : "/product_placeholder.svg"}
              ...
              <div>
                ...
                <p>
                  PKR {product?.product?.price?.toLocaleString()} * {product?.quantity} items
                </p>
              </div>
            </div>
          ))}
          <div className="flex flex-col gap-y-2 mt-10">
            <p className="text-2xl">Subtotal: PKR {order?.total?.toLocaleString()}</p>
            <p className="text-2xl">Tax 5%: PKR {Math.round(order?.total * 0.05).toLocaleString()}</p>
            <p className="text-2xl">Shipping: Free</p>
            <p className="text-3xl font-semibold">
              Total: PKR {Math.round(order?.total + order?.total * 0.05).toLocaleString()}
            </p>
          </div>
